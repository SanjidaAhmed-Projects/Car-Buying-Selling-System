...
public static class ConnectionClass
{
    ...
    static ConnectionClass()
    {
        string connectionString = 
            ConfigurationManager.ConnectionStrings["CarConnection"].ToString();
        conn = new SqlConnection(connectionString);
        command = new SqlCommand("", conn);
    }

    public static ArrayList GetCarByType(string carType)
    {
        ArrayList list = new ArrayList();
        string query = string.Format("SELECT * FROM car WHERE type LIKE '{0}'", carType);
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                Car car = new Car(id, name, type, price, roast, country, image, review);
                list.Add(car);
            }
        }
        finally
    ...
    {
        string query = String.Format("SELECT * FROM car WHERE id =  '{0}'", id);
        Car car = null;
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                car = new Car(name, type, price, roast, country, image, review);
            }
        }
        finally
        {
    ...
    public static void AddCar(Car car)
    {
        string query = string.Format(
            @"INSERT INTO car VALUES ('{0}', '{1}', @prices, '{2}', '{3}','{4}', '{5}')",
            car.Name, car.Type, car.Roast, car.Country, car.Image, car.Review);
        command.CommandText = query;
        command.Parameters.Add(new SqlParameter("@prices", car.Price));
        try
        {
            conn.Open();
    ...
    public static User LoginUser(string name, string password)
    {
        ...
        string query = string.Format("SELECT COUNT(*) FROM CarDB.dbo.users WHERE name = '{0}'", name);
        command.CommandText = query;
    ...
    {
        string query = string.Format(@"UPDATE [CarDB].[dbo].[orders]
                                       SET orderShipped = 1
                                       WHERE client = @client
                                       AND date = @date");
        try
